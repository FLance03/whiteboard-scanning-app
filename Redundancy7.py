...
labels, labelsInfo, textNonText, textLabels, wordLabels, phraseLabels, nonTextLabels = \
    file['labels'], file['labelsInfo'], file['textNonText'], file['textLabels'], file['wordLabels'], file['phraseLabels'], file['nonTextLabels']
...
for i in range(start, stop, ROW_OVERLAP_SIZE):
    ...
    featureData = ExtractFeatures(labels[i:i + 2*ROW_OVERLAP_SIZE, left:right+1], 139)
    features.append(np.concatenate((featureData[0], featureData[1]), axis=1))
if stop <= start:
    # The loop did not run even once
    featureData = ExtractFeatures(labels[top:bottom+1, left:right+1], 139)
    features.append(np.concatenate((featureData[0], featureData[1]), axis=1))
elif i + 1 < stop:
    ...
    featureData = ExtractFeatures(labels[i+ROW_OVERLAP_SIZE:bottom+1, left:right+1], 139)
    features.append(np.concatenate((featureData[0], featureData[1]), axis=1))
features = np.array(features, dtype=np.float32)
print(features)
print(features[0][5][5] == features[0][6][11] == 1)
numRows = bottom - top + 1
# testing.FullPrint(labels[top:bottom+1, left:right+1])
# testing.FullPrint(labelsInfo)
cv.imshow("this", testing.imshow_components(labels[top:bottom+1, left:right+1]))
